...
export function getStoragePublicUrl(filePath, bucketName = 'portfolio') {
    ...
    if (!client || !SUPABASE_CONFIG.url) {
        console.warn('Supabase не инициализирован, не могу сформировать URL');
        return filePath; // Возвращаем исходный путь как fallback
    }
    ...
    try {
        ...
        const publicUrl = `${SUPABASE_CONFIG.url}/storage/v1/object/public/${bucketName}/${cleanPath}`;
        return publicUrl;
    } catch (error) {
